...
import random
from models import *
from plots import plot_mean_preds_statistics, plot_stacked_horizontal_bars
...
def unchunk_and_plot(X_test, y_pred_probs, y_pred_loc, y_bin_labels, station_ids, window_size=CHUNK_SIZE):
    ...
    """
    positives = metrics['1_day']['TP'] + metrics['1_day']['FN']
    exact_TP = 100 / positives * metrics['1_day']['TP']
    window_TP = 100 / positives * (metrics['21_day']['TP']-metrics['1_day']['TP'])
    false_negative =  100 / positives * metrics['21_day']['FN']
    negatives = metrics['21_day']['TN'] + metrics['21_day']['FP']
    true_negative = 100 / negatives * metrics['21_day']['TN']
    false_positive = 100 / negatives * metrics['21_day']['FP']
    values = [exact_TP, window_TP, false_negative, true_negative, false_positive]
    plot_stacked_horizontal_bars(values, "mean_adjusted_bars")
     """

    # plot random stations
    selected_stations = random.sample(list(station_data.keys()), 10)
    for station_id in selected_stations:
        ...
        for i, label in enumerate(series_labels):
            ...
            if len(pred_pos) > 0:
                for pos in pred_pos:
                    ax.axvline(x=pos, color='black', linestyle='--', linewidth=1.5, label='Predicted position')
            

            # Secondary y-axis for summed predictions
            ax2 = ax.twinx()
            ax2.bar(days, summed_pred, alpha=0.4, color='orange', label= 'Total Score')
